...
@Tag(name = "Gestione ImmaginiRicetta", description = "Permette di gestire le ricette di lericettedinonnamaria")
@DenyAll
public class ImmaginiRicettaResources {
    ...
    @Operation(description = "Restituisce l'elenco di tutte le ricette")
    @APIResponses({
    ...
    public JsonArray allSlice() {
        //System.out.println(token);
        return storeimmaginiricetta.all().stream().map(User::toJsonSliceName).collect(JsonCollectors.toJsonArray());
    }
    ...
    public User find(@PathParam("id") Long id) {
        return storeimmaginiricetta.find(id).orElseThrow(() -> new NotFoundException("user non trovato. id=" + id));
    }
    ...
    public Response create(@Valid User entity) {
        ...
        if(storeimmaginiricetta.findImmaginiRicettabyNome(entity.getEmail()).isPresent()){
    ...
    public JsonObject login (@Valid Credential credential){
        ...
        ImmaginiRicetta u = storeimmaginiricetta.nome(credential).orElseThrow(() -> new NotAuthorizedException("User non Authorized",  
